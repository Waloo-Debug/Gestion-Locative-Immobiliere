...
export async function POST(request: Request) {
  ...
  if ((payment as RentPayment).status === "paid" && (payment as RentPayment).quittance_sent_at) {
    return NextResponse.json({ error: "Ce loyer est déjà confirmé et la quittance a été envoyée." }, { status: 400 });
  }
